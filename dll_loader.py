...
def wnd_proc(hwnd, msg, wparam, lparam): # 参数：窗口，返回消息，消息类型，消息指针
    ...
    if msg == My_Msg_StkData: # 通过验证msg确定是否是dll发送的数据
        ...
        if wFileType == RCV_REPORT:
            ...
            binary_output_file = f'{folder_path_raw}/{index}补报告数据{cuurent_time.replace(":", "")}.bin'
            # 初始化数据列表，批量写入的批次大小
            csv_list = []
            raw_list = []
                ...
                with open(output_file, mode='a', newline='') as file:
                    ...
                    for i in range(pHeader.contents.m_nPacketNum):
                        if pPower[i].m_head.m_dwHeadTag == EKE_HEAD_TAG:
                            ...
                            data_list.append([label,market,formatted_date,fGive,fPei,fPeiPrice,fProfit])
                        if len(data_list) >= batch_size:
                            writer.writerows(data_list)
                            data_list.clear()  # 清空列表准备下一个批次
                    if data_list:
...
def generate_raw_data_RCV_REPORT(buf, formatted_date):
    record = {
                ...
                "成交量": Buf.m_fVolume,
                "成交额": Buf.m_fAmount,
                "申买价1": Buf.m_fBuyPrice[0],
                "申买价2": Buf.m_fBuyPrice[1],
                "申买价3": Buf.m_fBuyPrice[2],
                "申买量1": Buf.m_fBuyVolume[0],
                "申买量2": Buf.m_fBuyVolume[1],
                "申买量3": Buf.m_fBuyVolume[2],
                "申卖价1": Buf.m_fSellPrice[0],
                "申卖价2": Buf.m_fSellPrice[1],
                "申卖价3": Buf.m_fSellPrice[2],
                "申卖量1": Buf.m_fSellVolume[0],
                "申卖量2": Buf.m_fSellVolume[1],
                "申卖量3": Buf.m_fSellVolume[2],
                "申买价4": Buf.m_fBuyPrice4,
                "申买量4": Buf.m_fBuyVolume4,
                "申卖价4": Buf.m_fSellPrice4,
                "申卖量4": Buf.m_fSellVolume4,
                "申买价5": Buf.m_fBuyPrice5,
                "申买量5": Buf.m_fBuyVolume5,
                "申卖价5": Buf.m_fSellPrice5,
                "申卖量5": Buf.m_fSellVolume5
            }
    return record
